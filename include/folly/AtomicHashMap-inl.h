...
AtomicHashMap(size_t finalSizeEst, const Config& config)
  ...
  assert(config.maxLoadFactor > 0.0 && config.maxLoadFactor < 1.0);
...
insertInternal(LookupKeyT key, ArgTs&&... vCtorArgs) {
 beginInsertInternal:
  ...
  if (tryLockMap(nextMapIdx)) {
    ...
    assert(subMaps_[nextMapIdx].load(std::memory_order_relaxed) ==
      (SubMap*)kLockedPtr_);
  ...
  assert(loadedMap && loadedMap != (SubMap*)kLockedPtr_);
...
findAtInternal(uint32_t idx) const {
  ...
  if (idx & kSecondaryMapBit_) {
    ...
    assert(subMapIdx < numMapsAllocated_.load(std::memory_order_relaxed));
...
AtomicHashMap<KeyT, ValueT, HashFcn, EqualFcn,
  ...
  assert(offset & kSecondaryMapBit_ == 0);  // offset can't be too big
  if (subMap == 0) return offset;
  // Make sure subMap isn't too big
  assert(subMap >> kNumSubMapBits_ == 0);
  // Make sure subMap bits of offset are clear
  assert(offset & (~kSubMapIndexMask_ | kSecondaryMapBit_) == 0);
